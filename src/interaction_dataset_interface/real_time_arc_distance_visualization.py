    ...
    while timestamp < timestamp_max:
        ...
        if visualize:
            ...
            if random_trackid in activeObjects.keys():
                ...
                for i in range(len(activeObjects[random_trackid].arcCoordinatesAlongPaths)):  # for each path
                    arc_distance = activeObjects[random_trackid].arcCoordinatesAlongPaths[i].distance
                    plt.scatter(timestamp, arc_distance,
                                c=colors[i], s=10, label='path arcCoordinate distance %s' % (i),marker=markers[i])
